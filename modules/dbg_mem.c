...
{
   os_printf( "[MEM_LEAK]\t(%s)\t%s(%d):\tfree\t\tat\t0x%08x\n",
...
void *dbg_malloc( size_t s, const char *file, unsigned line, bool use_iram )
{
   ...
   os_printf( "[MEM_LEAK]\t(%s)\t%s(%d):\tmallocate\t%d\tbytes at\t0x%08x\n",
...
{
   void *p = pvPortCalloc( l, s, "", line );
   os_printf( "[MEM_LEAK]\t(%s)\t%s(%d):\tcallocate\t%d\tbytes at\t0x%08x\n",
...
void *dbg_realloc( void *p, size_t s, const char *file, unsigned line )
{
   ...
   os_printf( "[MEM_LEAK]\t(%s)\t%s(%d):\treallocate\t%d\tbytes from\t0x%08x to 0x%08x \n",
...
{
   void *p = pvPortZalloc( s, "", line );
   os_printf( "[MEM_LEAK]\t(%s)\t%s(%d):\tzallocate\t%d\tbytes at\t0x%08x\n",
      sys_time2str( esp_log_timestamp() ), file, line, s, p );
   return p;
}
