...
// Changelog
//
//    2018-04-23  AWe   print out now with the line number
//    2018-04-06  AWe   file modified for compatibility with sources original
//                      written for ESP32 processors we use aweDBG.h which
//                      covers also the ESP_LOG* messages
//
// --------------------------------------------------------------------------
...
#ifdef USE_AWE_ESP_LOGx
   ESP_LOG_ALWAYS       // print log meassge independly from ESP_LOG_LEVEL
#endif
} esp_log_level_t;
...
#define LOG_COLOR_D
#define LOG_COLOR_V
#define LOG_COLOR__       LOG_BOLD( LOG_COLOR_CYAN )
 ...
 */
   ...
   #define ESP_LOG  ESP_LOGV
#else
   // my version
   #define ESP_LOG_LEVEL( level, tag, format, ... ) do {                     \
        if ( level==ESP_LOG_ERROR )          { esp_log_write( ESP_LOG_ERROR,      tag, LOG_FORMAT( E, " (%s) %s(%d): " format ), sys_time2str( esp_log_timestamp() ), tag, __LINE__, ##__VA_ARGS__ ); } \
        else if ( level==ESP_LOG_WARN )      { esp_log_write( ESP_LOG_WARN,       tag, LOG_FORMAT( W, " (%s) %s(%d): " format ), sys_time2str( esp_log_timestamp() ), tag, __LINE__, ##__VA_ARGS__ ); } \
        else if ( level==ESP_LOG_DEBUG )     { esp_log_write( ESP_LOG_DEBUG,      tag, LOG_FORMAT( D, " (%s) %s(%d): " format ), sys_time2str( esp_log_timestamp() ), tag, __LINE__, ##__VA_ARGS__ ); } \
        else if ( level==ESP_LOG_VERBOSE )   { esp_log_write( ESP_LOG_VERBOSE,    tag, LOG_FORMAT( V, " (%s) %s(%d): " format ), sys_time2str( esp_log_timestamp() ), tag, __LINE__, ##__VA_ARGS__ ); } \
        else                                 { esp_log_write( ESP_LOG_INFO,       tag, LOG_FORMAT( I, " (%s) %s(%d): " format ), sys_time2str( esp_log_timestamp() ), tag, __LINE__, ##__VA_ARGS__ ); } \
    } while( 0 )

    #define ESP_LOG( tag, format, ... )        esp_log_write( ESP_LOG_ALWAYS,     tag, LOG_FORMAT( _, " (%s) %s(%d): " format ), sys_time2str( esp_log_timestamp() ), tag, __LINE__, ##__VA_ARGS__ )
#endif
