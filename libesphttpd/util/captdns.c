...
// Changelog
//
//    2018-03-23  AWe   merged with https://github.com/espressif/ESP8266_IOT_PLATFORM/blob/master/libesphttpd/util/captdns.c
//    2018-03-23  AWe   update to chmorgan/libesphttpd
//                         hthttps://github.com/chmorgan/libesphttpd/commits/master
//                         Latest commit 7784b26 from Mar 10, 2018
//                         captdns.c - Improve logging to aid in debug
//                         captdns.c - Allow processing of dns requests with non-zero arcount values. Additional records can represent dns extensions and do not necessarily indicate dns responses that we'd like to ignore.
//                         captdns_task - Increase stack from 2900 to 3000 for slightly more headroom
// --------------------------------------------------------------------------

// see also https://github.com/cornelis-61/esp32_Captdns
...
static uint16_t ICACHE_FLASH_ATTR __ntohs( uint16_t in )
{
   char *p = ( char* )&in;
...
static void ICACHE_FLASH_ATTR captdnsRecv( struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length )
{
   ...
   ESP_LOGD( TAG, "DNS packet: id 0x%X flags 0x%X rcode 0x%X qcnt %d ancnt %d nscount %d arcount %d len %d",
             ntohs( hdr->id ), hdr->flags, hdr->rcode, ntohs( hdr->qdcount ),
             ntohs( hdr->ancount ), ntohs( hdr->nscount ), ntohs( hdr->arcount ), length );
   ...
   if( hdr->ancount || hdr->nscount || hdr->arcount ) // this is a reply, don't know what to do with it
   {
   ...
   for( i = 0; i < ntohs( hdr->qdcount ); i++ )
   {
      ...
      p = labelToStr( pusrdata, p, length, buf, DNS_LEN );
      if( p == NULL )
         goto finish;

      DnsQuestionFooter *qf = ( DnsQuestionFooter* )p;
      p += sizeof( DnsQuestionFooter );
      ESP_LOGD( TAG, "DNS: Q ( type 0x%X class 0x%X ) for %s", ntohs( qf->type ), ntohs( qf->class ), buf );
      if( ntohs( qf->type ) == QTYPE_A )
      {
         // They want to know the IPv4 address of something.
         // Build the response.
         rend = strToLabel( buf, rend, DNS_LEN - ( rend - reply ) ); // Add the label
         if( rend == NULL )
            goto finish;
         ...
         *rend++ = ip4_addr4( &info.ip );
         setn16( &rhdr->ancount, ntohs( rhdr->ancount ) + 1 );   // increment ancount
      ...
      else if( ntohs( qf->type ) == QTYPE_URI )
      {
         ...
         setn16( &uh->prio, 10 );
         setn16( &uh->weight, 1 );
         memcpy( rend, "http://esp.nonet", 16 );  // !!! todo ( AWe ): change URL
         rend += 16;
         setn16( &rhdr->ancount, ntohs( rhdr->ancount ) + 1 );
         ESP_LOGD( TAG, "Added NS rec to resp. Resp len is %d", ( rend - reply ) );
      }
...
static void captdnsTask( void *pvParameters )
{
   ...
   ESP_LOGI( TAG, "CaptDNS initialized" );
   while( 1 )
   {
      ...
      if( ret > 0 )
      {
         ESP_LOGD( TAG, "captdns recv %d Bytes from Port %d %s", ret, ntohs( from.sin_port ), inet_ntoa( from.sin_addr ) );
         captdnsRecv( &from, udp_msg, ret );
      }

      ESP_LOGD( TAG, "captdns stack %d, heap %d", ( int )uxTaskGetStackHighWaterMark( NULL ), esp_get_free_heap_size() );
   }
...
void ICACHE_FLASH_ATTR captdnsInit( void )
{
   ...
   conn.proto.udp->local_port = DNS_SERVER_PORT;
